...
from src.models import TruePairwiseLTR, ManifoldTruePairwiseLTR
...
def main(
    ...
    print('Prepare data')
    _, train_val_loader, val_loader, test_loader = get_loaders(*get_data(data_dir, data_name, "bag_of_items_negatives", num_negatives=(num_negatives, max(ks) - 1)), batch_size, num_workers)
    ...
    manifold = geoopt.Euclidean() if manifold == 'euc' else geoopt.PoincareBall(1.0)

    model = ManifoldTruePairwiseLTR(emb_dim, manifold, num_items, 1, dtype=dtype).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--manifold", type=str, choices=['euc', 'poincare'])
