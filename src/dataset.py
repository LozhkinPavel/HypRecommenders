...
class UserItemWithNegativesDataset(Dataset):
    def __init__(self, interactions_df: pd.DataFrame, item_df: pd.DataFrame, num_negatives: int = 1, shuffle: bool = True, seed: int = 42):
        ...
        self.interactions = torch.sparse_coo_tensor(np.stack((interactions_df['user_id'].values, interactions_df['item_id'].values), axis=0), ones, (self.num_users, self.num_items)).to_dense()
    ...
    def __getitem__(self, index):
        ...
        items_interactions = self.interactions[:, torch.cat((torch.tensor([pos_item_id]), neg_item_id))].T
