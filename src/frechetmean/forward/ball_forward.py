...
from src.frechetmean.utils import EPS, arcosh
...
def frechet_ball_forward(X, w, K=-1.0, max_iter=200, rtol=1e-6, atol=1e-6, verbose=False):
    """
    Args
    ----
    ...
    for _ in range(max_iter):
        ...
        mu = torch.where(b_ss[..., None] != 0, eta.unsqueeze(-1) * b, torch.zeros_like(b))
        assert torch.all(~torch.isnan(mu))

        dist = (mu - mu_prev).norm(dim=-1)
        prev_dist = mu_prev.norm(dim=-1)
        if (dist < atol).all() or (dist < rtol * prev_dist).all():
            break
